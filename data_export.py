...
try:
    ...
    for row_num, row in enumerate(dataframe_to_rows(data, index=False, header=False), start=fila_inicio_tabla + 1):
        for col_num, value in enumerate(row, start=1):
            ...
            if data.iloc[row_num - fila_inicio_tabla - 1]["ID Servicio"] in [9, 10]:
                cell.fill = PatternFill(start_color="FFA500", end_color="FFA500", fill_type="solid")
            # Pintar de color rosa las filas con ID Servicio 11
            elif data.iloc[row_num - fila_inicio_tabla - 1]["ID Servicio"] in [11]:
                cell.fill = PatternFill(start_color="FF1493", end_color="FF1493", fill_type="solid")
            # Pintar de color azul las celdas con "No gestante" en la columna "Gestante"
            elif data.columns[col_num - 1] == "Gestante" and value == "No gestante":
    ...
    archivo_excel = 'atenciones_estilizadas_con_encabezado.xlsx'
